...
export async function sendVerificationNotification(to, {
  ...
  if (!to) throw new Error('Recipient number (to) is required for verification notification');
  const normalized = /^57\d{10}$/.test(to) ? to : `57${to.replace(/\D/g, '').replace(/^0+/, '')}`;
  if (!/^57\d{10}$/.test(normalized)) {
    throw new Error(`Invalid normalized phone number: ${normalized}`);
  }
  ...
  switch(status) {
    case 'pending':
      ...
      parameters = [
        { type: 'text', text: String(nombreEmpleado || 'Empleado').trim() },
        { type: 'text', text: String(montoFormato || '$0').trim() },
        { type: 'text', text: String(referencia || 'N/A').trim() }
      ];
      break;
    ...
    case 'confirmed':
      ...
      parameters = [
        { type: 'text', text: String(nombreEmpleado || 'Empleado').trim() },
        { type: 'text', text: String(montoFormato || '$0').trim() },
        { type: 'text', text: String(nombreBanco || 'Banco').trim() },
        { type: 'text', text: String(fechaTransaccion || '').trim() },
        { type: 'text', text: String(referencia || 'N/A').trim() }
      ];
      break;
    ...
    case 'error':
      ...
      parameters = [
        { type: 'text', text: String(nombreEmpleado || 'Empleado').trim() },
        { type: 'text', text: String(montoFormato || '$0').trim() },
        { type: 'text', text: String(razonRechazo || 'No se pudo verificar').trim() },
        { type: 'text', text: String(referencia || 'N/A').trim() }
      ];
      break;
  ...
  if (!res.ok) {
    const errBody = await res.text();
    const errorMsg = `Meta API error: status=${res.status} body=${errBody.slice(0, 500)}`;
    console.error(`[whatsapp/template] ERROR from Meta: ${errorMsg}`);
    console.error(`[whatsapp/template] Details: to=${normalized} templateName=${templateName} status=${status}`);
