...
export async function getCompanySubscription(companyId) {
  const { data, error } = await supabaseAdmin
    .from('companies')
    .select('id, name, plan, subscription_status, trial_ends_at, is_active, max_employees, max_verifications_month, max_bank_accounts, max_admin_alerts, max_admin_numbers, alerts_sent_month, alerts_reset_at')
    .eq('id', companyId)
    .maybeSingle();
...
export async function checkLocationLimit(companyId) {
  ...
  if (count >= (company.max_locations ?? 1)) {
    const plan = PLANS[company.plan];
    const limit = company.max_locations ?? 1;
    return {
      ok: false,
      reason: `Tu plan ${plan?.label || company.plan} permite máximo ${limit} sede${limit === 1 ? '' : 's'}. Actualiza tu plan para agregar más.`,
      limitReached: true
    };
  }
  return { ok: true };
