 ...
 * PLANS es la fuente de verdad; la BD puede tener valores desactualizados.
 */
export async function checkEmployeeLimit(companyId) {
  ...
  const effectiveMax = planDef?.maxEmployees ?? company.max_employees;

  if (count >= effectiveMax) {
    return {
      ok: false,
      reason: `Tu plan ${planDef?.label || company.plan} permite máximo ${effectiveMax} empleado${effectiveMax === 1 ? '' : 's'} activo${effectiveMax === 1 ? '' : 's'}. Actualiza tu plan para agregar más.`,
      limitReached: true
    };
  }
  return { ok: true };
...
export async function checkBankAccountLimit(companyId) {
  ...
  const planDef    = PLANS[company.plan];
  const effectiveMax = planDef?.maxBankAccounts ?? company.max_bank_accounts;

  if (count >= effectiveMax) {
    return {
      ok: false,
      reason: `Tu plan ${planDef?.label || company.plan} permite máximo ${effectiveMax} cuenta${effectiveMax === 1 ? '' : 's'} bancaria${effectiveMax === 1 ? '' : 's'}. Actualiza tu plan para agregar más.`,
      limitReached: true
    };
  }
  return { ok: true };
...
export async function checkLocationLimit(companyId) {
  ...
  const planDef = PLANS[company.plan];
  const maxLoc = company.max_locations ?? planDef?.maxLocations ?? 1;
  if (count >= maxLoc) {
    return {
      ok: false,
      reason: `Tu plan ${planDef?.label || company.plan} permite máximo ${maxLoc} sede${maxLoc === 1 ? '' : 's'}. Actualiza tu plan para agregar más.`,
      limitReached: true
    };
  }
  return { ok: true };
...
export async function checkVerificationLimit(companyId) {
  ...
  const planDef    = PLANS[company.plan];
  const effectiveMax = planDef?.maxVerificationsMonth ?? company.max_verifications_month;

  const used = await getMonthlyVerificationCount(companyId);
  if (used >= effectiveMax) {
    return {
      ok: false,
      reason: `Alcanzaste el límite de ${effectiveMax} verificaciones este mes (plan ${planDef?.label || company.plan}). Renueva o actualiza tu plan.`,
      limitReached: true,
      used,
      max: effectiveMax
    };
  }
  return { ok: true, used, max: effectiveMax };
}
