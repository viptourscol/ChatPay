...
export async function matchTransaction({ amount, reference, date, senderName, companyId }) {
  ...
  if (reference) {
    ...
    if (alreadyConfirmed) {
      // Si la referencia existe pero el monto del comprobante es diferente → FALSIFICADO
      if (amount && Number(alreadyConfirmed.amount) !== Number(amount)) {
  ...
  if (!tx && amount) {
    ...
    if (byAmount?.length === 1) {
      ...
      // Ventana progresiva: primero ±10 min (match exacto), luego ±30 min, luego ±90 min
      // Así, si el comprobante tiene hora precisa (ej. 09:53), no mezcla con 09:41
      let candidates = null;
      for (const mins of [10, 30, 90]) {
        const lo = new Date(ocrUtc.getTime() - mins * 60 * 1000).toISOString();
        const hi = new Date(ocrUtc.getTime() + mins * 60 * 1000).toISOString();
        const inWindow = byAmount.filter(row =>
          row.transaction_date >= lo && row.transaction_date <= hi
        );
        if (inWindow.length === 1) { tx = inWindow[0]; break; }
        if (inWindow.length > 0 && candidates === null) candidates = inWindow;
      }
      // Si ninguna ventana dio resultado único, usar la más estrecha con resultados
      ...
      if (!tx && candidates.length === 1) {
        ...
        if (best.score > 0 && best.score > (second?.score ?? 0)) {
          ...
          if (smsTx) {
            ...
            // Antes de declarar ambigüedad, verificar si ya fue confirmado recientemente
            // (mismo monto, últimas 24h). Cubre el caso de reenvíos sin referencia OCR.
            if (amount) {
              const center = date
                ? new Date(new Date(date).getTime() + COLOMBIA_OFFSET_MS)
                : new Date();
              const lo24 = new Date(center.getTime() - 24 * 60 * 60 * 1000).toISOString();
              const hi24 = new Date(center.getTime() + 24 * 60 * 60 * 1000).toISOString();
  ...
  if (tx) {
    // Guardar también la referencia OCR si la transacción no la tenía.
    // Crítico para detectar reenvíos como duplicado en la siguiente verificación.
  ...
  if (reference) {
    ...
    if (confirmed) {
      if (amount && Number(confirmed.amount) !== Number(amount)) {
