...
export async function requireCompany(userId, res) {
  ...
  // Suscripción activa pero fecha de vencimiento ya pasó → suspender automáticamente
  if (company.subscription_status === 'active' && company.subscription_expires_at) {
    if (new Date(company.subscription_expires_at) < new Date()) {
      // Actualizar estado en DB
      await supabaseAdmin.from('companies').update({ subscription_status: 'suspended', is_active: false }).eq('id', company.id);
