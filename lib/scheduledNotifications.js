...
/**
 * Obtiene comprobantes pendientes con detalles individuales, agrupados por sede.
 * 
 * Retorna: [{location_id, location_name, total_amount, verifications: [...]}, ...]
 * Cada verificación incluye: {id, employee_name, amount, created_at}
 * 
 * @param {string} companyId - UUID de la empresa
 * @param {UUID[]} locationIds - IDs de sedes a incluir (null = todas)
 * @returns {Promise<Array>} Array de ubicaciones con detalles de cada comprobante
 */
export async function getLocationPendingVerifications(companyId, locationIds = null) {
  ...
  // Agrupar por sede, pero mantener detalles de cada comprobante
  const locationMap = {};
  ...
  for (const v of verifications || []) {
    ...
    locationMap[locationId].verifications.push({
      id: v.id,
      employee_name: v.employees.name || 'Sin empleado',
      amount: v.extracted_amount || 0,
  ...
  // Ordenar sedes por cantidad de comprobantes (descendente)
  result.sort((a, b) => b.verifications.length - a.verifications.length);

  return result;
}

/**
 * Formatea un mensaje WhatsApp con detalles individuales de cada comprobante pendiente.
 * 
 * @param {Array} locationStats - Array de { location_name, total_amount, verifications: [{employee_name, amount, created_at}, ...] }
...
export function formatNotificationMessage(locationStats) {
  ...
  for (const location of locationStats) {
    totalPending += location.verifications.length;
    totalAmount += location.total_amount;

    const locationEmoji = location.verifications.length > 0 ? '🏢' : '✅';
    message += `${locationEmoji} *${location.location_name}*\n`;
    message += `   (${location.verifications.length} comprobante${location.verifications.length !== 1 ? 's' : ''})\n\n`;

    // Mostrar cada comprobante individualmente
    for (let i = 0; i < location.verifications.length; i++) {
      const v = location.verifications[i];
      const verTime = new Date(v.created_at);
      const timeFormatted = verTime.toLocaleTimeString('es-CO', {
        hour: '2-digit',
        minute: '2-digit'
      });
      const dateFormatted = verTime.toLocaleDateString('es-CO', {
        month: '2-digit',
        day: '2-digit'
      });

      message += `   ${i + 1}. ${v.employee_name}\n`;
      message += `      💰 $${Number(v.amount).toLocaleString('es-CO')} | ⏰ ${timeFormatted} (${dateFormatted})\n\n`;
    }

    message += `   📊 Subtotal Sede: $${Number(location.total_amount).toLocaleString('es-CO')}\n\n`;
  }

  // Resumen total
  message += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  message += `🔴 *TOTAL: ${totalPending} comprobante${totalPending !== 1 ? 's' : ''} pendiente${totalPending !== 1 ? 's' : ''}*\n`;
