...
/**
 * Dado un email To header, extrae el alias local (antes del @) y busca la empresa.
 * Soporta: "empresa1@chatpay.co", "Nombre <empresa1@chatpay.co>"
 * Si no hay match, retorna el id del primer tenant (retrocompatibilidad).
 */
async function resolveCompanyFromTo(toHeader) {
  if (!toHeader) return null;
  const match = toHeader.match(/([a-z0-9_.-]+)@/i);
...
export async function syncBancolombiaEmails({ minutes = 60 } = {}) {
  ...
  // Fallback: id del único tenant (retrocompatibilidad con setup pre-SaaS)
  ...
  for (const m of messages) {
    ...
    const toHdr   = headers.find((h) => h.name === 'To')?.value || '';
    ...
    // Identificar empresa por el alias del destinatario del email
    const companyId = (await resolveCompanyFromTo(toHdr)) || fallbackCompanyId;
