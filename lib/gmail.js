...
async function resolveCompanyFromHeaders(headers) {
  ...
  // Buscar empresa que tenga alguno de esos emails como bancolombia_email
  ...
  for (const company of data) {
    if (candidates.has(company.bancolombia_email?.toLowerCase())) {
      return company.id;
    }
  }
  return null;
...
export async function syncBancolombiaEmails({ minutes = 60 } = {}) {
  ...
  for (const m of messages) {
    ...
    if (parsed.isDebit) continue; // Es un débito (dinero saliendo), no registrar
