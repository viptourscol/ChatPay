...
const BANCOLOMBIA_QUERY =
  'from:(bancolombia.com.co OR alertasynotificaciones@notificacionesbancolombia.com)';
...
export function parseBancolombiaEmail({ subject = '', body = '', date }) {
  ...
  // Monto: $123.456,78  ó  $ 1,234,567  ó  COP 50.000
  ...
  const senderMatch =
    text.match(/de\s+([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑa-záéíóúñ\s]{3,40}?)(?:\s+por|\s+con|\s+a\s+tu|\s+cuenta|\s+\$|,|\.|$)/) ||
...
export async function syncBancolombiaEmails({ minutes = 60 } = {}) {
  ...
  const q = `from:(bancolombia.com.co OR alertasynotificaciones@notificacionesbancolombia.com) after:${after}`;
  ...
  try {
    ...
    for (const msgId of messageIds) {
      try {
        ...
        // Solo procesar emails de Bancolombia
        if (!from.toLowerCase().includes('bancolombia')) continue;
