...
import { reconcilePendingVerifications } from './reconcilePendingVerifications.js';
...
function decodeQP(str) {
  // Quoted-Printable: unir líneas soft-wrapped (=\r\n o =\n), luego decodificar =XX
  return str
    .replace(/=\r\n/g, '').replace(/=\n/g, '')
...
function getBodyText(msg) {
  ...
  // Detectar si el part usa quoted-printable y decodificarlo
  const enc = (textPart.headers || []).find(h => h.name?.toLowerCase() === 'content-transfer-encoding')?.value || '';
  if (enc.toLowerCase().includes('quoted-printable') || /=[0-9A-Fa-f]{2}/.test(body)) {
    body = decodeQP(body);
  }

  if (textPart.mimeType === 'text/html') {
    body = body.replace(/<[^>]+>/g, ' ').replace(/&nbsp;/g, ' ').replace(/&[a-z]+;/gi, ' ');
  }
...
export function parseBancolombiaEmail({ subject = '', body = '', date }) {
  ...
  const debitPatterns = [
    ...
    /tu\s+compra/i,
    /compra\s+(?:de|por|en)/i,
  ];

  // Si dice "recibiste un pago" o "recibiste una transferencia" → siempre es ingreso,
  // sin importar qué otros patrones haya en el texto (el encabezado "Listo! Todo salió bien
  // con tus movimientos Bancolombia" aparece tanto en ingresos como en egresos).
  const creditOverride = /recibiste\s+(?:un\s+pago|una\s+transferencia)/i.test(text);
  const isDebit = !creditOverride && debitPatterns.some((p) => p.test(text));
  ...
  // Destinatario: nombre propio "a NOMBRE el" o número de cuenta "a la cuenta *XXXX el"
  const receiverMatch =
    text.match(/cuenta\s+\*\d+\s+a\s+([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑA-Za-z\s]{3,50}?)\s+el\s/i) ||
    text.match(/transferiste[^a]+a\s+(?:la\s+llave\s+\S+\s+[^\n]*?a\s+)([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑ\s]{3,50}?)\s+el\s/i) ||
    text.match(/destinatario[:\s]+([A-ZÁÉÍÓÚÑ][^\n.,]{3,60})/i);
  // Si no hay nombre, intentar capturar el número de cuenta destino
  const accountMatch = !receiverMatch &&
    text.match(/a\s+la\s+cuenta\s+\*(\d+)/i);
  const receiver_name = receiverMatch
    ? receiverMatch[1].trim()
    : accountMatch
      ? `Cuenta *${accountMatch[1]}`
      : null;
...
async function resolveCompanyFromHeaders(headers) {
  ...
  console.log('[gmail/resolveCompany] candidatos emails:', [...candidates]);
...
export async function syncBancolombiaEmails({ minutes = 60 } = {}) {
  console.log(`[gmail/sync] starting: minutes=${minutes}`);
  ...
  console.log(`[gmail/sync] found ${messages.length} emails`);
  ...
  for (const m of messages) {
    ...
    console.log(`[gmail/parse] id=${m.id} isDebit=${parsed.isDebit} amount=${parsed.amount} receiver=${parsed.receiver_name} body200=${body.slice(0,200).replace(/\n/g,'↵')}`);
    ...
    if (parsed.isDebit) {
      ...
      // Deduplicar con SMS: si ya existe un egreso SMS con mismo monto y fecha (±1 día), enriquecerlo en lugar de duplicar
      const payDate = parsed.transaction_date
        ? new Date(parsed.transaction_date).toISOString().slice(0, 10)
        : new Date().toISOString().slice(0, 10);
      const dayBefore = new Date(new Date(payDate).getTime() - 86400000).toISOString().slice(0, 10);
      const dayAfter  = new Date(new Date(payDate).getTime() + 86400000).toISOString().slice(0, 10);
      const { data: smsEgreso } = await supabaseAdmin
        .from('egresos')
        .select('id')
      ...
      if (smsEgreso) {
        // Enriquecer el egreso SMS con datos del email, marcar source como 'gmail'
        await supabaseAdmin.from('egresos').update({
          source: 'gmail',
      ...
      // No hay egreso SMS previo → insertar nuevo egreso desde Gmail
      const { error: insertEgresoErr } = await supabaseAdmin.from('egresos').insert({
      ...
      if (insertEgresoErr) console.error('[gmail/egreso] insert error:', insertEgresoErr.message);
      continue;
    }

    // Identificar empresa por bancolombia_email (revisa To, X-Forwarded-To, Delivered-To)
    const companyId = (await resolveCompanyFromHeaders(headers)) || fallbackCompanyId;    // Deduplicar: evitar doble registro cuando SMS ya creó la transacción
    if (!companyId) {
      console.log(`[gmail/parse] skipped: no companyId found for email id=${m.id}`);
      continue;
    }

    affectedCompanyIds.add(companyId);
    console.log(`[gmail/parse] added companyId=${companyId} to affected set, now size=${affectedCompanyIds.size}`);

    // Deduplicar: evitar doble registro cuando SMS ya creó la transacción
    // o cuando Bancolombia envía dos emails del mismo pago.
    // Ventana ±2 horas para cubrir desfase entre SMS y email.
    const txDate = new Date(parsed.transaction_date);
    const lo = new Date(txDate.getTime() - 2 * 60 * 60 * 1000).toISOString();
    const hi = new Date(txDate.getTime() + 2 * 60 * 60 * 1000).toISOString();
    const { data: existing } = await supabaseAdmin
      ...
      .maybeSingle();
    if (existing) {
      // Si fue creada por SMS, enriquecerla con datos del email (sender, ref, gmail_id)
      const patch = {};
      if (!existing.source || existing.source === 'sms') {
        if (parsed.reference_number) patch.reference_number = parsed.reference_number;
        if (parsed.sender_name)      patch.sender_name      = parsed.sender_name;
        patch.raw_subject      = subject;
        patch.raw_snippet      = msg.snippet;
        patch.gmail_message_id = msg.id;
        if (Object.keys(patch).length) {
          await supabaseAdmin.from('transactions').update(patch).eq('id', existing.id);
        }
      }
      // Vincular SMS huérfanos al id existente
      await supabaseAdmin.from('transaction_sms')
        .update({ transaction_id: existing.id, status: 'linked' })
        .eq('company_id', companyId)
        .eq('status', 'pending_match')
        .or(`reference.eq.${parsed.reference_number || '__none__'},amount.eq.${parsed.amount}`);
    ...
    const { error, data: upserted } = await supabaseAdmin
      ...
      // Vincular SMS huérfanos que esperaban este email
      if (upserted?.id) {
        const orFilter = parsed.reference_number
          ? `reference.eq.${parsed.reference_number},amount.eq.${parsed.amount}`
          : `amount.eq.${parsed.amount}`;
        await supabaseAdmin
          .from('transaction_sms')
          .update({ transaction_id: upserted.id, status: 'linked' })
          .eq('company_id', companyId)
          .eq('status', 'pending_match')
          .or(orFilter);
      }
  ...
  for (const companyId of affectedCompanyIds) {
    console.log(`[gmail/reconcile] calling reconcilePendingVerifications for companyId=${companyId}`);
    try {
      const result = await reconcilePendingVerifications({ companyId, source: 'gmail' });
      console.log(`[gmail/reconcile] result:`, result);
    } catch (reconcileErr) {
      console.error('[gmail/reconcile] error:', reconcileErr.message, reconcileErr.stack || '');
    }
  }

  console.log(`[gmail] syncFromGmail completed: scanned=${messages.length} inserted=${inserted} companiesProcessed=${affectedCompanyIds.size}`);
