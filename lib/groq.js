...
const LOG_OCR_DEBUG = process.env.LOG_OCR_DEBUG === 'true';
...
Regla critica para el campo amount:
- Si el comprobante muestra tanto "Monto total" como "Monto" y además aparece un impuesto, comisión, 4x1000, GMF o cargo adicional, usa como amount el valor de "Monto" o el valor transferido real.
- No uses como amount el total debitado cuando incluya impuestos o cargos adicionales.
- Ejemplo: si ves "Monto total $30.120", "Monto $30.000" e "Impuesto 4x1.000 $120", el amount correcto es 30000.
...
export async function extractComprobanteData(imageUrl) {
  let lastError = null;
  const maxRetries = 2;  // Reintentar hasta 2 veces por modelo

  for (const model of OCR_MODELS) {
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        const rawContent = await runOcrCompletion(model, imageUrl);
        if (LOG_OCR_DEBUG) console.log('[groq] raw OCR response received');
        ...
        if (LOG_OCR_DEBUG) {
          console.log('[groq] normalized fields:', JSON.stringify({
            hasAmount: normalized.amount != null,
            hasReference: normalized.reference != null,
            hasDate: normalized.date != null,
            hasSender: normalized.sender_name != null,
            isReceipt: normalized.is_receipt === true,
          }));
        }
        return normalized;
      } catch (error) {
        lastError = error;
        const attemptStr = attempt < maxRetries ? ` (reintentando ${attempt + 1}/${maxRetries})` : '';
        console.warn(`[groq] OCR falló con ${model}${attemptStr}:`, error?.message || error);
...
async function coerceToReceiptJson(model, rawContent) {
  try {
    ...
    if (LOG_OCR_DEBUG) console.log('[groq] repaired OCR response received');
...
async function runOcrCompletion(model, imageUrl) {
  if (OCR_PROVIDER === 'openrouter') {
    ...
    if (!content) {
      const errorMsg = data?.error?.message || 'OpenRouter OCR returned empty response';
...
async function runOpenRouterChatCompletion({ model, messages }) {
  ...
  // Si tenemos una respuesta válida con contenido, usarla aunque HTTP status no sea perfecto
  if (data?.choices?.[0]?.message?.content) {
    if (LOG_OCR_DEBUG && !response.ok) {
      console.warn(`[groq] OpenRouter HTTP ${response.status} but got valid response content`);
    }
    return data;
  }

  // Si no hay contenido válido, entonces es un error
  if (!response.ok) {
    const message = data?.error?.message || `OpenRouter HTTP ${response.status}`;
    const details = data?.error?.details || '';
    console.error(`[groq] OpenRouter error: ${message} ${details}`);
    throw new Error(message);
  }

  // HTTP 200 pero sin contenido
  throw new Error('OpenRouter returned 200 but no valid response content');
}}
