    ...
    try {
      ...
      if (isJsonValidateFailed(error)) {
        try {
          ...
          const rawContent = completion.choices?.[0]?.message?.content ?? '{}';
          console.log('[groq] retry raw OCR response:', rawContent);
          const parsed = parseGroqJson(rawContent);
          console.log('[groq] retry parsed:', JSON.stringify(parsed));
          const normalized = normalizeExtraction(parsed);
          console.log('[groq] retry normalized:', JSON.stringify(normalized));
          return normalized;
        } catch (retryError) {
