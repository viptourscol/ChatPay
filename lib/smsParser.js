...
export function parseBancolombiaSmS(text, receivedAt) {
  ...
  // Detectar débitos (dinero saliendo)
  const debitMatch = /transferiste|pagaste|compra(?:ste)?|retir(?:aste|[oó])|d[eé]bito|pago\s+a\s+/i.test(t);

  let amount = null, senderName = null, reference = null, recipientName = null;

  if (debitMatch) {
    // Débito: extraer monto y destinatario
    // "transferiste $X a la llave Y" | "Pagaste $X a NOMBRE" | "Compraste $X en NOMBRE" | "Retiraste $X en ATM"
    const debitAmtM = t.match(/(?:transferiste|pagaste|compraste|retiraste|pago\s+a\s+\S+\s+por)\s+\$([\.\d,]+)/i)
                   || t.match(/\$([\d.,]+)/);
    if (debitAmtM) amount = parseAmount(debitAmtM[1]);
    // Para retiros en cajero: extraer lugar/ATM como destinatario
    const retiroM = t.match(/retiraste\s+\$[\d.,]+\s+en\s+([A-Z0-9]+)/i);
    const recipM = retiroM
      ? [null, retiroM[1]]
      : t.match(/(?:a\s+(?:la\s+llave\s+|la\s+cuenta\s+\*?)?([\S]+)|a\s+([A-ZÁÉÍÓÚÑ][^.$\n]{3,50?})(?:\s+por|\s+en|\s*$))/i);
    if (recipM) recipientName = (recipM[1] || recipM[2])?.trim() || null;
    if (!amount) return null;
    const refMD = t.match(/(?:ref(?:erencia)?|n[uú]mero|no\.?)[^\d]*([A-Z0-9]{4,})/i);
    if (refMD) reference = refMD[1].trim();
    return {
      bank: 'Bancolombia',
      amount,
      reference,
  ...
  // ── Formato Llaves/Transferencia: "recibiste un pago/transferencia de NOMBRE por $X.XX" ──────────────
  // Captura: "recibiste un pago de...", "recibiste una transferencia de..."
  const llavesM = t.match(/recibiste\s+(?:un\s+pago|una\s+transferencia)\s+de\s+([^p]+?)\s+por\s+\$\s*([\d.,]+)/i);
  if (llavesM) {
