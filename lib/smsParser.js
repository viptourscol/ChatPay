...
export function parseBancolombiaSmS(text, receivedAt) {
  ...
  const debitMatch = /transferiste|pagaste|compra(?:ste)?|retir[oó]|d[eé]bito|pago\s+a\s+/i.test(t);

  let amount = null, senderName = null, reference = null, recipientName = null;

  if (debitMatch) {
    // Débito: extraer monto y destinatario
    // "transferiste $X a la llave Y" | "Pagaste $X a NOMBRE" | "Compraste $X en NOMBRE"
    const debitAmtM = t.match(/(?:transferiste|pagaste|compraste|pago\s+a\s+\S+\s+por)\s+\$([\d.,]+)/i)
                   || t.match(/\$([\d.,]+)/);
    if (debitAmtM) amount = parseAmount(debitAmtM[1]);
    const recipM = t.match(/(?:a\s+(?:la\s+llave\s+|la\s+cuenta\s+\*?)?(\S+)|a\s+([A-ZÁÉÍÓÚÑ][^.$\n]{3,50?})(?:\s+por|\s+en|\s*$))/i);
