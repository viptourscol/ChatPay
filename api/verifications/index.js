...
export default async function handler(req, res) {
  ...
  // Se ejecuta vía cron job diario/horario. Procesa tareas de expiración de DB.
  // No requiere autenticación de usuario (es automático).
  if (req.query.action === 'expire-pending' && req.method === 'POST') {
    ...
    try {
      console.log('[expire-pending] Iniciando procesamiento de expiración');
      
      // Llamar función que procesa todas las tareas vencidas usando la tabla verification_expiration_tasks
      const result = await processAllExpiredVerifications();

      console.log('[expire-pending] Procesamiento completo:', result);
      return res.json({
        ok: true,
        ...result
      });
